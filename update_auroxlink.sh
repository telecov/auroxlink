...
if ! command -v curl > /dev/null 2>&1; then
  echo "❌ ERROR: El comando 'curl' no está instalado. Ejecuta: sudo apt install curl"
  exit 1
fi
...
# Validar si se obtuvo correctamente
...
echo "===> Paso 3: Descargando AUROXLINK desde $ZIP_URL"
cd /tmp || exit 1
wget "$ZIP_URL" -O "$ZIP_NAME"
if [ ! -f "$ZIP_NAME" ]; then
  echo "❌ ERROR: No se pudo descargar el archivo $ZIP_NAME"
  exit 1
fi
...
# Paso 6: Restaurar personalizados
...
# Paso 12: Actualizar version.txt
